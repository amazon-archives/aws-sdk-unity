...
namespace Amazon.MobileAnalyticsManager.Delivery.Internal
{
    internal class SQLiteEventStore:IEventStore
    {
        ...
        /// <summary>
        /// Initializes the <see cref="Amazon.MobileAnalytics.Delivery.SQLiteEventStore"/> class.
        /// </summary>
        ...
        private static void SetUpDatabase(String dbPath)
        {
            lock (SQLiteDatabase.SQLiteLock)
            {
                ...
                {
                    AmazonLogging.LogException("SQLiteEventStore",e);
                }
                finally
                {
                    if (stmt != null)
        ...
        public bool PutEvent(string eventString, string appId)
        {
            ...
            if(currentDatabaseSize>=_maxDbSize)
            {
                ...
                AmazonLogging.LogError("SQLiteEventStore","The database size has exceeded the threshold limit. Unable to insert any new events");
            }
            else if(currentDatabaseSize/_maxDbSize >= _dbWarningThreshold)
            {
                proceedToInsert = true;
                AmazonLogging.LogWarn("SQLiteEventStore","The database size is almost full");
            }
            else
            {
                proceedToInsert = true;
